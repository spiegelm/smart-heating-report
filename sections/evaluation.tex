...
\begin{itemize}
	...
	\item We discovered a invalid temperature value in the heating schedule which is rejected by the thermostat. 1244 out of 1957 time outs are related to this invalid value.
\end{itemize}
...
Due to time constraints we could not collect the same amount of data as for the first evaluation.
...
The success rate of 40.9 percent increased to 98.6 percent.
...
PUT /set/target	& 15	& 15	& 4	& 10	& 26.7 \% \\
			\bottomrule
		\end{tabular}
		\begin{tablenotes}
			\footnotesize{
...
A closer look at the data indicates that the timed out \highlight{PUT /set/target} requests did still do their work.
Although when the gateway did not receive a response, the requested target temperature is often returned by the subsequent GET query.
